...
def recover_secret_key(pk, messages, signatures):
    ...
    for i, s in enumerate(signatures):
        signatures[i] = pow(signatures[i], 2**(hash_length * (no_rows - i - 1)), pk.n)

    print("Reconstructing the secret key values...")
    r = product([pow(signatures[j], rows[0][j]) for j in xrange(len(signatures))], pk.n)
    s = product([pow(signatures[j], rows[1][j]) for j in xrange(len(signatures))], pk.n)

    secret_key = SignatureScheme.SecretKey(int(pk.n), int(pk.max_version), hash_length, int(r), int(s))
...
def attack(target, target_pk, remote_pk, messages, signatures):
    ...
    sig_scheme.sk.s = int(sig_scheme.sk.s)
...
if __name__ == "__main__":
    ...
    target = ("35.198.143.105", int(2024))
